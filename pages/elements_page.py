...
class ElementsPage(BasePage):
    ...
    def print_all_info_about_profile(self):
        ...
        for i in range(len(tags_names)):
            ...
            allure_text.append(f'{tags_names[i].inner_text()} {tags_values[i].inner_text()}')
        ...
        for i in self.links_repos:
            print(i.inner_text())
            allure_text.append(f'{i.inner_text()}')
        allure.attach('\n'.join(allure_text), 'Текст со страницы', allure.attachment_type.TEXT)
